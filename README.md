...
- **Modernized stack** - oclif v4, native fetch, cross-platform open/copy, upgraded dependencies
...
That's it. For day-to-day development, run TypeScript directly - no build step needed:
```sh-session
$ ./bin/dev.js            # launches interactive site manager
$ ./bin/dev.js ls         # or any command
```
...
To use `local-cli` globally from anywhere:
```sh-session
$ npm link
$ local-cli               # launches interactive site manager
$ local-cli ls            # or any command
```
<!-- usagestop -->
